...
class RetrievingEventResource extends JsonResource
{
    /**
    ...
    public function toArray($request)
    {
        ...
        return [
            ...
            'amounts' =>  generateRandomTargetableAmounts('events', $this->funded), //TEMPORARY
            "liked_by_auth" => $donor ? $donor->likes()->where(['likeable_type' => 'event' , 'likeable_id' => $this->id])->exists() : false,
            "funded_by_auth" => $faker->boolean(), //TEMPORARY
            "saved_by_auth" => $donor ? $donor->savedItems()->where(['saveable_type' => 'event' , 'saveable_id' => $this->id])->exists() : false,
