...
<script>
        ...
        return {
            config: function (config, successCallback = null) {
                ...
                return {
                    ...
                    setByAction: function (action) {
                        this.config.method = (action === 'add') ? 'POST' : 'PUT';
                    },
    ...
    for (i = 0; i < routes.length; i++) {
        if (typeof window[routes[i].controller_name + 'Init'] == 'undefined') window[routes[i].controller_name + 'Init'] = function () {
            return null;
        };
        ...
        $routeProvider.otherwise({
            ...
            resolve: {
                $currentRoute: function ($page) {
                    $page.set({routeName: "{{ $r['name'] }}", controllerName: "{{ $r['controller_name'] }}"});
                },
                $init: eval("{{ $r['controller_name'] . 'Init' }}"),
            },
        });
