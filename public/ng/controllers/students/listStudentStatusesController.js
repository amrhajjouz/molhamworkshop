async function listStudentStatusesControllerInit(
  $http,
  $page,
  $apiRequest,
  $datalist
  ...
  const statuses = await $apiRequest
    .config("students/" + $page.routeParams.id + "/statuses")
    .getData();
...
function listStudentStatusesController($scope, $page, $apiRequest, $init) {
  $scope.statuses = $init.statuses;

 $scope.createUpdateStatusContent = $apiRequest.config(
   {
     method: "POST",
     url: `students/${$page.routeParams.id}/statuses`,
     data: $scope.status,
   },
      ...
      switch (action) {
        ...
        case "edit":
          $scope.selectedStatus = data;
              ...
              $scope.createUpdateStatusContent.config.url = `api/students/${$page.routeParams.id}/statuses/${$scope.selectedStatus.id}/contents`;
              $scope.contents = {};
