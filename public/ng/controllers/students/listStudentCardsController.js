async function listStudentCardsControllerInit(
  $http,
  $page,
  $apiRequest,
  $datalist
  ...
  const cards = await $apiRequest
    .config("students/" + $page.routeParams.id + "/cards")
    .getData();
...
function listStudentCardsController($scope, $page, $apiRequest, $init) {
  $scope.cards = $init.cards;

  $scope.createUpdateCard = $apiRequest.config(
    {
      method: "POST",
      url: `students/${$page.routeParams.id}/cards`,
