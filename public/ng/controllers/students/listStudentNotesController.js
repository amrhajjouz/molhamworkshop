async function listStudentNotesControllerInit(
  $http,
  $page,
  $apiRequest,
  $datalist
  ...
  const notes = await $apiRequest
    .config("students/" + $page.routeParams.id + "/notes")
    .getData();
...
function listStudentNotesController($scope, $page, $apiRequest, $init) {
  $scope.notes = $init.notes;

  $scope.createUpdateNote = $apiRequest.config(
    {
      method: "POST",
      url: `students/${$page.routeParams.id}/notes`,
  ...
  $scope.showModal = function (action, data = {}) {
    ...
    switch (action) {
      case "add":
        ...
        $scope.createUpdateNote.config.url = `api/students/${$page.routeParams.id}/notes`;
        break;
      case "edit":
        $scope.selectedNote = angular.copy(data);
        $scope.createUpdateNote.config.method = action = "PUT";
        $scope.createUpdateNote.config.url = `api/students/${$page.routeParams.id}/notes/${$scope.selectedNote.id}`;
        break;
      default:
        break;
    }
  ...
  $scope.addDeleteReview = async (note) => {
    ...
    let userExists = false;
      ...
      if(userExists){
        reviewUrl = `students/${$page.routeParams.id}/notes/${note.id}/unreview`;
      }else{
        reviewUrl = `students/${$page.routeParams.id}/notes/${note.id}/review`;
      }
