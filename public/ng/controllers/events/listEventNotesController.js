async function listEventNotesControllerInit(
  $http,
  $page,
  $apiRequest,
  $datalist
  ...
  const notes = await $apiRequest
    .config("events/" + $page.routeParams.id + "/notes")
    .getData();
...
function listEventNotesController($scope, $page, $apiRequest, $init) {
  $scope.notes = $init.notes;

  $scope.createUpdateNote = $apiRequest.config(
    {
      method: "POST",
      url: `events/${$page.routeParams.id}/notes`,
  ...
  $scope.showModal = function (action, data = {}) {
    ...
    switch (action) {
      case "add":
        ...
        $scope.createUpdateNote.config.url = `api/events/${$page.routeParams.id}/notes`;
        break;
      case "edit":
        $scope.selectedNote = angular.copy(data);
        $scope.createUpdateNote.config.method = action = "PUT";
        $scope.createUpdateNote.config.url = `api/events/${$page.routeParams.id}/notes/${$scope.selectedNote.id}`;
        break;
      default:
        break;
    }
  ...
  $scope.addDeleteReview = async (note) => {
    ...
    let userExists = false;
      ...
      if(userExists){
        reviewUrl = `events/${$page.routeParams.id}/notes/${note.id}/unreview`;
      }else{
        reviewUrl = `events/${$page.routeParams.id}/notes/${note.id}/review`;
      }
