async function listEventCardsControllerInit(
  $http,
  $page,
  $apiRequest,
  $datalist
  ...
  const cards = await $apiRequest
    .config("events/" + $page.routeParams.id + "/cards")
    .getData();
...
function listEventCardsController($scope, $page, $apiRequest, $init) {
  $scope.cards = $init.cards;

  $scope.createUpdateCard = $apiRequest.config(
    {
      method: "POST",
      url: `events/${$page.routeParams.id}/cards`,
