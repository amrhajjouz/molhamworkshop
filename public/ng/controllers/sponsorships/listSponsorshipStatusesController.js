async function listSponsorshipStatusesControllerInit(
  $http,
  $page,
  $apiRequest,
  $datalist
  ...
  const statuses = await $apiRequest
    .config("sponsorships/" + $page.routeParams.id + "/statuses")
    .getData();
...
function listSponsorshipStatusesController($scope, $page, $apiRequest, $init) {
  $scope.statuses = $init.statuses;

 $scope.createUpdateStatusContent = $apiRequest.config(
   {
     method: "POST",
     url: `sponsorships/${$page.routeParams.id}/statuses`,
     data: $scope.status,
   },
      ...
      switch (action) {
        ...
        case "edit":
          $scope.selectedStatus = data;
              ...
              $scope.createUpdateStatusContent.config.url = `api/sponsorships/${$page.routeParams.id}/statuses/${$scope.selectedStatus.id}/contents`;
              $scope.contents = {};
