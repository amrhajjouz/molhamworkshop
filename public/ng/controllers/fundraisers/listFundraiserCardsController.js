async function listFundraiserCardsControllerInit(
  $http,
  $page,
  $apiRequest,
  $datalist
  ...
  const cards = await $apiRequest
    .config("fundraisers/" + $page.routeParams.id + "/cards")
    .getData();
...
function listFundraiserCardsController($scope, $page, $apiRequest, $init) {
  $scope.cards = $init.cards;

  $scope.createUpdateCard = $apiRequest.config(
    {
      method: "POST",
      url: `fundraisers/${$page.routeParams.id}/cards`,
