async function listFundraiserStatusesControllerInit(
  $http,
  $page,
  $apiRequest,
  $datalist
  ...
  const statuses = await $apiRequest
    .config("fundraisers/" + $page.routeParams.id + "/statuses")
    .getData();
...
function listFundraiserStatusesController($scope, $page, $apiRequest, $init) {
  $scope.statuses = $init.statuses;

 $scope.createUpdateStatusContent = $apiRequest.config(
   {
     method: "POST",
     url: `fundraisers/${$page.routeParams.id}/statuses`,
     data: $scope.status,
   },
      ...
      switch (action) {
        ...
        case "edit":
          $scope.selectedStatus = data;
              ...
              $scope.createUpdateStatusContent.config.url = `api/fundraisers/${$page.routeParams.id}/statuses/${$scope.selectedStatus.id}/contents`;
              $scope.contents = {};
