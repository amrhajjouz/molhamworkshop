...
function listUserPermissionsController($scope, $init, $page, $apiRequest) {
  ...
  $scope.selectedPermission = {permissions_ids: null, user_id: $scope.userId};
  $scope.unassignPermission = async (permissionID) => {
    if (!confirm('هل تريد التأكيد على إلغاء إسناد هذه الصلاحية ؟ ')) return;
    const data = {user_id: $scope.userId,permission_id: permissionID,};
    $apiRequest.config({method: 'POST',url: 'users/' + $scope.userId + '/unassign_permission',data: data,},function (response, data) {$page.reload();}).send();
  };

  $scope.assignPermission = $apiRequest.config({method: 'POST',url: 'users/' + $scope.userId + '/assign_permissions',data: $scope.selectedPermission,},
    function (response, data) {
      $('#permission-modal').on('hidden.bs.modal', function (e) {$page.reload();});
