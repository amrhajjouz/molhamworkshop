async function listBlogNotesControllerInit(
  $http,
  $page,
  $apiRequest,
  $datalist
  ...
  const notes = await $apiRequest
    .config("blogs/" + $page.routeParams.id + "/notes")
    .getData();
...
function listBlogNotesController($scope, $page, $apiRequest, $init) {
  $scope.notes = $init.notes;

  $scope.createUpdateNote = $apiRequest.config(
    {
      method: "POST",
      url: `blogs/${$page.routeParams.id}/notes`,
  ...
  $scope.showModal = function (action, data = {}) {
    ...
    switch (action) {
      case "add":
        ...
        $scope.createUpdateNote.config.url = `api/blogs/${$page.routeParams.id}/notes`;
        break;
      case "edit":
        $scope.selectedNote = angular.copy(data);
        $scope.createUpdateNote.config.method = action = "PUT";
        $scope.createUpdateNote.config.url = `api/blogs/${$page.routeParams.id}/notes/${$scope.selectedNote.id}`;
        break;
      default:
        break;
    }
  ...
  $scope.addDeleteReview = async (note) => {
    ...
    let userExists = false;
      ...
      if(userExists){
        reviewUrl = `blogs/${$page.routeParams.id}/notes/${note.id}/unreview`;
      }else{
        reviewUrl = `blogs/${$page.routeParams.id}/notes/${note.id}/review`;
      }
