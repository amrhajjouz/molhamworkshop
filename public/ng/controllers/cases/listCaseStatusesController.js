async function listCaseStatusesControllerInit(
  $http,
  $page,
  $apiRequest,
  $datalist
  ...
  const statuses = await $apiRequest
    .config("cases/" + $page.routeParams.id + "/statuses")
    .getData();
...
function listCaseStatusesController($scope, $page, $apiRequest, $init) {
  $scope.statuses = $init.statuses;

 $scope.createUpdateStatusContent = $apiRequest.config(
   {
     method: "POST",
     url: `cases/${$page.routeParams.id}/statuses`,
     data: $scope.status,
   },
      ...
      switch (action) {
        ...
        case "edit":
          $scope.selectedStatus = data;
              ...
              $scope.createUpdateStatusContent.config.url = `api/cases/${$page.routeParams.id}/statuses/${$scope.selectedStatus.id}/contents`;
              $scope.contents = {};
