...
function listCaseAttachmentsController($scope, $page, $apiRequest, $init) {
  ...
  //     appendPre('Files:');
  //     var files = $scope.files = response.result.files;
  ...
  //             // id, name, webContentLink, webViewLink
  //           })
  //           .then(
  //             function (success) {

  //               $scope.driveFiles.push(angular.copy(success));
  ...
  //         $scope.$evalAsync();
  //             // console.log('ssssss' , $scope.driveFiles);
  ...
  function listDriveFiles(q) {
    // "0AK-4shZBKhfWUk9PVA";//root folder
    // q = `mimeType = "application/vnd.google-apps.root"`; //view just folders
