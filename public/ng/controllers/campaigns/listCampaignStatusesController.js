async function listCampaignStatusesControllerInit(
  $http,
  $page,
  $apiRequest,
  $datalist
  ...
  const statuses = await $apiRequest
    .config("campaigns/" + $page.routeParams.id + "/statuses")
    .getData();
...
function listCampaignStatusesController($scope, $page, $apiRequest, $init) {
 ...
 $scope.createUpdateStatusContent = $apiRequest.config(
   {
     method: "POST",
     url: `campaigns/${$page.routeParams.id}/statuses`,
     data: $scope.status,
   },
      ...
      switch (action) {
        ...
        case "edit":
          $scope.selectedStatus = data;
              ...
              $scope.createUpdateStatusContent.config.url = `api/campaigns/${$page.routeParams.id}/statuses/${$scope.selectedStatus.id}/contents`;
              $scope.contents = {};
