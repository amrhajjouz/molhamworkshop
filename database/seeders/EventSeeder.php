...
class EventSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        for ($i = 1; $i <= 20; $i++) {
            $event = Event::create([
                'date' => date('Y-m-d H:i:s' , rand(strtotime('2021-01-01'), time())),
                'donor_id' => $faker->numberBetween($min = 1, $max = 30),
                'verified' => $faker->boolean(),
                'public_visibility' => $faker->boolean(),
                'implemented' => $faker->boolean(),
                'implementation_date' => date('Y-m-d H:i:s' , rand(strtotime('2021-01-01'), time())),
                'youtube_video_url' => $faker->url(),
            ...
            $event->target->updateContentFields($contents);
            $event->target->update(['published_at' => date('Y-m-d H:i:s' , rand(strtotime('2021-01-01'), time()))]);
        }
    }
}
